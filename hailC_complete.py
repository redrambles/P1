...
def buildCoder(shift):
    ...
    lowerShift = zip(lower, (lower[shift:] + lower[:shift]))
    upperShift = zip(upper, (upper[shift:] + upper[:shift]))

    coder = dict(lowerShift + upperShift)
    return coder
...
def findBestShift(wordList, text):
    ...
    for i in range(0, 26):
        ...
        # Shift the entire text by this shift and make it into split list.
        list = applyShift(text, i).split(' ')
        count = 0
